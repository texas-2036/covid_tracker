...
tsa_data <- read_excel("data/dshs/TSA COVID Bed Reports_05.10.20.xlsx", 
...
tsa_data_vents <- read_excel("data/dshs/TSA COVID Bed Reports_05.10.20.xlsx", 
  ...
  write_csv(tsa_data,"clean_data/dshs/hospitals/tsa_bed_data_with_counties_sf.csv")
  write_csv(tsa_data_vents,"clean_data/dshs/hospitals/tsa_vent_data_with_counties_sf.csv")
...
syndromic_tx <- read_excel("data/dshs/TxS2 Chart May 3 Dashboard.xlsx", sheet="Texas", skip=0) %>%
  clean_names() %>% 
  mutate(date = ymd(date)) %>% 
  write_csv("clean_data/dshs/syndromic_tx.csv")

syndromic_cli <- read_excel("data/dshs/TxS2 Chart May 3 Dashboard.xlsx", sheet="CLI Data", skip=0) %>%
  clean_names() %>% 
  mutate(date = ymd(date)) %>% 
  write_csv("clean_data/dshs/syndromic_cli.csv")

syndromic_ili <- read_excel("data/dshs/TxS2 Chart May 3 Dashboard.xlsx", sheet="ILI Data", skip=0) %>%
  clean_names() %>% 
  mutate(date = ymd(date)) %>% 
...
syndromic_tx <- read_excel("data/dshs/NEW 5.10.2020.3PM COVID-19 Roadmap to Recovery Dashboard.xlsx", 
    sheet = "Texas DASHBOARD", skip = 11) %>%
  clean_names() %>% 
  select(date=1,ili_from_syndromic_surveillance=8,cli_from_syndromic_surveillance=10) %>% 
  mutate(date=as.numeric(date)) %>% 
  mutate(date = as.Date(date, origin = "1899-12-30")) %>% 
  filter(!is.na(ili_from_syndromic_surveillance)) %>% 
  write_csv("clean_data/dshs/syndromic_tx.csv")

syndromic_cli <- read_excel("data/dshs/NEW 5.10.2020.3PM COVID-19 Roadmap to Recovery Dashboard.xlsx", 
    sheet = "CLI Symptoms DASHBOARD", skip = 3) %>%
  clean_names() %>% 
  select(-1) %>% 
  gather(date, value, 2:12) %>% 
  mutate(date = gsub(pattern="^x", replacement="",x=date)) %>% 
  mutate(date=as.numeric(date)) %>% 
  mutate(date = as.Date(date, origin = "1899-12-30")) %>% 
  write_csv("clean_data/dshs/syndromic_cli.csv")

syndromic_ili <- read_excel("data/dshs/NEW 5.10.2020.3PM COVID-19 Roadmap to Recovery Dashboard.xlsx", 
    sheet = "ILI Symptoms DASHBOARD", skip = 3)  %>%
  clean_names() %>% 
  select(-1) %>% 
  gather(date, value, 2:12) %>% 
  mutate(date = gsub(pattern="^x", replacement="",x=date)) %>% 
...
library(aws.s3)
Sys.setenv(
  "AWS_ACCESS_KEY_ID"     = "AKIAWWZ7POZOPTKH2WSQ",
  "AWS_SECRET_ACCESS_KEY" = "uut24/FaUFdlX7vMuDiCbY4FGH3fEskt+Cyqc6eJ",
  "AWS_DEFAULT_REGION"    = "us-west-2")

safegraph_raw <- s3read_using(read.csv,
...
twc_claims_cnty <- read_excel(paste0("data/twc/",today,"_", lcl), skip=2) %>%
  slice(1:254) %>% 
  clean_names() %>% 
  filter(!is.na(county)) %>% 
  remove_empty("cols") %>% 
  ...
  mutate(date=gsub("^x","0",x=date),
         date=gsub("_","-",x=date),
         date=gsub("20202$","2020",x=date)) %>% 
  mutate(date=lubridate::mdy(date))

twc_claims_all_cnty <- twc_claims_cnty %>% 
  filter(str_detect(date,"^2020")) %>% 
  mutate(state="TX") %>% 
  group_by(state) %>% 
  tally(value)

write_csv(twc_claims_cnty,paste0("clean_data/twc/county_claims_data_",today,".csv"))
write_csv(twc_claims_cnty,paste0("clean_data/twc/county_claims_data.csv"))
...
## TWC UI by Zip & County 

```{r}

twc_ui_by_county <- read_csv("https://lmci.state.tx.us/shared/dashboarddata/ui_by_county.csv")

twc_cnty_naics1 <- twc_ui_by_county %>% 
  select(1:17) %>% 
  select(-naics1,-naics2,-naics3,-naics4,-naics5) %>% 
  select(1:2, 3:4) %>% 
  rename(naics_title=3, naics_count=4) %>% 
  mutate(rank=1)

twc_cnty_naics2 <- twc_ui_by_county %>% 
  select(1:17) %>% 
  select(-naics1,-naics2,-naics3,-naics4,-naics5) %>% 
  select(1:2, 5:6) %>% 
  rename(naics_title=3, naics_count=4) %>% 
...
twc_cnty_naics3 <- twc_ui_by_county %>% 
  select(1:17) %>% 
  select(-naics1,-naics2,-naics3,-naics4,-naics5) %>% 
  select(1:2, 7:8) %>% 
  rename(naics_title=3, naics_count=4) %>% 
...
twc_cnty_naics4 <- twc_ui_by_county %>% 
  select(1:17) %>% 
  select(-naics1,-naics2,-naics3,-naics4,-naics5) %>% 
  select(1:2, 9:10) %>% 
  rename(naics_title=3, naics_count=4) %>% 
...
twc_cnty_naics5 <- twc_ui_by_county %>% 
  select(1:17) %>% 
  select(-naics1,-naics2,-naics3,-naics4,-naics5) %>% 
  select(1:2, 11:12) %>% 
  rename(naics_title=3, naics_count=4) %>% 
...
twc_cnty_claims_long <- bind_rows(twc_cnty_naics1,
                                  twc_cnty_naics2,
                                  twc_cnty_naics3,
                                  twc_cnty_naics4,
                                  twc_cnty_naics5) %>% 
  mutate(pct_of_all=naics_count/ui_count,
         pct_label=naics_count/ui_count) %>% 
  select(1:3,rank,naics_count,pct_of_all,pct_label) %>% 
  mutate_at(vars(pct_label),scales::percent_format(scale=100,accuracy=.1)) %>% 
  select(-ui_count,-naics_count,-pct_of_all) %>% 
  write_csv("clean_data/twc/county_claims_by_industry_detail.csv")

twc_ui_by_zip <- read_csv("https://lmci.state.tx.us/shared/dashboarddata/ui_by_zipcode.csv")
...
## Homebase S3 Bucket Data
```{r}

homebase_s3_raw <- read_csv("data/homebase/20200101_to_20200411_hours_data.csv") %>% 
  filter(state=="tx")

homebase_s3_tx <- homebase_s3_raw %>% 
  separate(event_date, into=c("year", "month", "day"), sep="-") %>% 
  unite("year_mo", c("year","month"),sep = "-") 

tx_employees_by_county <- homebase_s3_tx %>%
  filter(level=="Employee") %>% 
  select(year_mo, county_code, user_id, total_wages_earned) %>% 
  filter(!is.na(total_wages_earned)) %>% 
  mutate(county_code = as.character(county_code),
         user_id = as.character(user_id)) %>% 
  group_by(year_mo, county_code) %>% 
  count(user_id) %>% 
  group_by(year_mo, county_code, user_id) %>% 
  ungroup() %>% 
  select(-n) %>% 
  mutate(count=1) %>% 
  group_by(year_mo, county_code) %>% 
  summarise(employees=sum(count)) %>% 
  arrange(year_mo, county_code) %>% 
  ungroup()


tx_wages_by_county <- homebase_s3_tx %>%
  filter(level=="Employee") %>% 
  select(year_mo, county_code, level, total_wages_earned) %>%
  filter(!is.na(total_wages_earned)) %>% 
  mutate(county_code = as.character(county_code)) %>% 
  group_by(year_mo, county_code) %>% 
  summarise(county_tot_wages = sum(total_wages_earned)) %>% 
  left_join(tx_employees_by_county, by =c("year_mo","county_code")) %>% 
  filter(!is.na(employees)) %>% 
  mutate(avg_wages = county_tot_wages/employees)
...
```{r}
tx_wages_by_county %>% 
  ggplot(aes(x=year_mo, y=avg_wages)) +
  geom_col() +
  texas2036::theme_2036() +
...
tx_series_all <-fredr(
  ...
  add_row(date="2020-05-02", series_id="TXICLAIMS", value=247179) %>%
