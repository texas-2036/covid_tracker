...
test_positive <- test_daily %>% 
  ...
  arrange(date) %>% 
  select(date, state, positive, totalTestResults, positiveIncrease, negativeIncrease) %>% 
  mutate(daily_test_pos_rate = round(positive/totalTestResults, digits=4),
...
test_positive %>% 
  ...
  theme_2036() +
  theme(legend.position =c(.9,.51)) +
  scale_y_continuous(labels = scales::percent_format(accuracy=1), breaks=c(0,.01,.02,.03,.04,.05,.06,.07,.08,.09,.1), limits=c(0,.12)) +
...
tsa_data <- read_excel("data/dshs/TSA COVID Bed Reports_04.21.20.xlsx", skip=0) %>%
  clean_names() %>% 
  filter(!is.na(tsa)) %>% 
  left_join(crosswalk_data, by="tsa") %>% 
  select(1,2,tsa_counties,everything()) %>% 
  separate_rows(tsa_counties) %>% 
  st_as_sf() %>% 
  write_rds("clean_data/dshs/hospitals/tsa_hosp_data_with_counties_sf.rds")

write_csv(tsa_data,"clean_data/dshs/hospitals/tsa_hosp_data_with_counties_sf.csv")
```
