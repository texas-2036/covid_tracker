...
dshs_state_ages <- read_excel(lcl, sheet = "Cases by Age Group", skip=1) %>% 
  clean_names() %>% 
  select(group=age, cases=count, pct=3) %>% 
  filter(pct<1) %>% 
...
tsa_data <- read_excel("data/dshs/TSA COVID Bed Reports 04.24.20.xlsx", 
  ...
  st_as_sf() %>% 
  write_rds("clean_data/dshs/hospitals/tsa_bed_data_with_counties_sf.rds")

tsa_data_vents <- read_excel("data/dshs/TSA COVID Bed Reports_04.23.20.xlsx", 
  ...
  mutate(tsa_counties=str_trim(tsa_counties, side="both")) %>% 
  st_as_sf() %>% 
  write_rds("clean_data/dshs/hospitals/tsa_vent_data_with_counties_sf.rds")


  write_rds(tsa_data,paste0("clean_data/dshs/hospitals/tsa_bed_data_with_counties_sf_",today,".rds"))
  write_rds(tsa_data,paste0("clean_data/dshs/hospitals/tsa_vent_data_with_counties_sf_",today,".rds"))
...
syndromic_tx <- read_excel("data/dshs/TxS2 Chart April 26 Dashboard.xlsx", sheet="Texas", skip=0) %>%
  clean_names() %>% 
  mutate(date = ymd(date)) %>% 
  write_csv("clean_data/dshs/syndromic_tx.csv")

syndromic_cli <- read_excel("data/dshs/TxS2 Chart April 26 Dashboard.xlsx", sheet="CLI Data", skip=0) %>%
  clean_names() %>% 
  mutate(date = ymd(date)) %>% 
  write_csv("clean_data/dshs/syndromic_cli.csv")

syndromic_ili <- read_excel("data/dshs/TxS2 Chart April 26 Dashboard.xlsx", sheet="ILI Data", skip=0) %>%
  clean_names() %>% 
  mutate(date = ymd(date)) %>% 
